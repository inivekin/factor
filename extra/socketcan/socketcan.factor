...
ui.gadgets.panes ui.gadgets.scrollers ui.gadgets.search-tables
ui.gadgets.tables ui.gadgets.tracks ui.theme ui.tools.common
unix.ffi unix.ffi.linux unix.time unix.types vectors ;
...
:: (candump-pane) ( datagram ts flag -- )
    <pane> t >>scrolls? :> p
    datagram ts flag
    '[ p <scroller> "" open-window p [ [
      [ _ receive-canmsg now duration>seconds >float _ - [ print-candump-frame ] with-string-writer write nl ]
      [ dup io-timeout? [ rethrow ] unless drop ] recover
      _ input-flag-raised?
    ] loop ] with-pane ] in-thread ;
: candump-pane ( datagram -- )
    1 seconds over set-timeout
    now duration>seconds >float <flag> [ (candump-pane) ] keep
    enter-pressed? raise-flag ;
...
: <cantable> ( channel -- stop-flag table )
    ...
    updating-id-list get [ keys ] <arrow> 0.1 seconds <decimate> cantable-renderer [ ] <search-table> white-interior
    ...
    grid canframe unique-str-id updating-id-list get value>> at
        {
          [ 0.1 seconds <decimate>
                [ canframe>> timestamp>> [ timestamp>unix-time >float "%.6f" sprintf ] [ "0.0" ] if* ] <?arrow> <label-control> default-monospace-font-name <font> >>font ]
          [ 0.1 seconds <decimate>
                [ dt>> duration>seconds >float "%.6f" sprintf ] <?arrow> <label-control> default-monospace-font-name <font> >>font ]
          [ 0.1 seconds <decimate>
                [ count>> number>string ] <?arrow> <label-control> default-monospace-font-name <font> >>font ]
          [ 0.1 seconds <decimate>
                [ addrspec>> channel>> ] <?arrow> <label-control> default-monospace-font-name <font> >>font ]
          [ 0.1 seconds <decimate>
                [ canframe>> data>> length number>string ] <?arrow> <label-control> default-monospace-font-name <font> >>font ]
          [ 0.1 seconds <decimate>
                [ [ canframe>> (print-id) ] with-string-writer ] <?arrow> <label-control> default-monospace-font-name <font> >>font ]
          [ 0.1 seconds <decimate>
                [ canframe>> data>> [ "%02X" sprintf ] { } map-as " " join ] <?arrow> <label-control> default-monospace-font-name <font> >>font ]
        } { } cleave>sequence
